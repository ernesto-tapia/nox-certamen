...
export interface ProgrammedAction { faction:FactionId; cardId:string; speed:number; slot:number; targetId?:number; originIds?:number[]; }
...
export interface GameState { version:1; seed:string; rngState:number; round:number; phase:Phase; playerFaction:FactionId; factions:Record<FactionId,Faction>; territories:Territory[]; cards:CardDefinition[]; program:ProgrammedAction[]; botPrograms:ProgrammedAction[]; revealedPrograms:ProgrammedAction[]; resolutionQueue:ProgrammedAction[]; currentActionIndex:number; resolutionOriginId:number|null; resolutionTargetId:number|null; reaction:Reaction|null; events:GameEvent[]; publicObjectives:Objective[]; objectiveDeck:Objective[]; technologies:Technology[]; winner:FactionId|null; selectedTerritory:number|null; }
export interface SavedGame { version:1; savedAt:string; state:GameState; }
export type GameCommand = {type:'START'; faction:FactionId; seed:string}|{type:'SELECT_TERRITORY'; territoryId:number}|{type:'SELECT_RESOLUTION_ORIGIN'; territoryId:number}|{type:'SELECT_RESOLUTION_TARGET'; territoryId:number}|{type:'ADD_CARD'; cardId:string}|{type:'REMOVE_CARD'; cardId:string}|{type:'SET_REACTION'; cardId:string}|{type:'COMMIT_PROGRAM'}|{type:'RESOLVE_NEXT_ACTION'}|{type:'RESOLVE_ROUND'}|{type:'ADVANCE_PHASE'}|{type:'RESET'};
